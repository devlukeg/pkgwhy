...
`pkgwhy` is in the `1.6.0` release line. Public JSON schemas and rule IDs are compatibility surfaces; incompatible changes require changelog coverage and may require a schema or catalog version bump.
...
- `1.6.0`: agent decision-contract hardening, JSON error objects, batch precheck summaries, registry trust-state JSON, trust-aware tool judgement, local tool validation, static capability analysis for verified tool bundles, and `pkgwhy agent check`.

...
Tracked public files must not contain local work-loop files or provider traces:

```bash
git ls-files | grep -E '(^\.agent/|^AGENTS\.md$|^AGENT_WORK_ORDER\.md$|^\.codex/)' || true
git grep -nE "OpenAI|ChatGPT|Codex|CodeRabbit|AGENTS\.md|AGENT_WORK_ORDER|AI assistant|generated by|model-generated|\.codex" -- . || true
```
...
- Run CodeRabbit only after local checks pass and the diff is coherent.
